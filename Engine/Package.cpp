...
bool Package::Save(const fs::path& path) const
{
	...
	const auto isScriptStructNotEmpty = [&gen = generator](ScriptStruct&& s) -> bool
	{
		return !s.Members.empty() || !s.PredefinedMethods.empty();
	};

	const auto isClassNotEmpty = [&gen = generator](Class&& c) -> bool
	{
		return !c.Members.empty() || !c.Methods.empty() || !c.PredefinedMethods.empty() 
				|| (gen->ShouldGenerateEmptyFilesWithStaticClass() && !c.StaticClass.Body.empty());
	};
	...
	if (generator->ShouldGenerateEmptyFiles()
		...
		|| from(scriptStructs) >> where(isScriptStructNotEmpty) >> any()
		|| from(classes) >> where(isClassNotEmpty) >> any()
		)
	)
	{
		SaveEnums(path);
...
void Package::GenerateClass(const UEClass& classObj)
{
	...
	{
		c.StaticClass = IGenerator::PredefinedMethod::Inline(tfm::format(R"(	static UClass* StaticClass()
	{
...
void Package::SaveStructs(const fs::path& path) const
{
	...
	if(!enums.empty())
	{
		includes.push_back(GenerateFileName(FileContentType::Enums, *this));
	}
...
void Package::SaveEnums(const fs::path& path) const
{
	extern IGenerator* generator;

	if (!enums.empty())
	{
		std::ofstream os(path / GenerateFileName(FileContentType::Enums, *this));
...
void Package::PrintClass(std::ostream& os, const Class& c) const
{
	...
	if(!c.PredefinedMethods.empty() || !c.StaticClass.Body.empty())
	{
		const auto printPredefinedMethod = [&outStream = os](auto&& m) -> void
		{
			if (m.MethodType == IGenerator::PredefinedMethod::Type::Inline)
			{
				outStream << m.Body;
		...
		//StaticClass Method
		if(!c.StaticClass.Body.empty())
		{
			printPredefinedMethod(c.StaticClass);
		}
